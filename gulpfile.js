...
const SRC_PATH = "src";
const SRC_HOST = "local.bradball.net";
...
gulp.task('compress-images', ['clean'], function() {
	...
	var src_files = [
		...
		SRC_PATH+"/_/img/favicon/**/*.+(jpg|jpeg|png|gif|webp|bmp)",
		"!"+SRC_PATH+"/_/bower/**/*.*"
	];
	...
	var filterFavicons = $.filter(SRC_PATH+"/_/img/favicon/**/*.*", {restore: true});
	...
	gulp.src(src_files)
		...
		.pipe(filterFavicons)
		.pipe($.rename({dirname: "_/img/favicon"}))
